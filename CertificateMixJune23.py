...
################################################################################
### Nombre del script: CertificateMixJune.py
### Autor: Adrián Payol Montero
### Equipo de trabajo: WAF Telefónica
### Versión: 1.0
### Explicación: Salida a excels individuales por entidad con 2 hojas
...
IMPERVA_API_ID = "xxxxxx"
IMPERVA_API_KEY = "yyyyyyy"
...
#Parseo del argumento de la entrada el account id
...
def getCnameRevalidationStatus (siteId, accountId, siteName):
    ...
    if len(response_dict["data"][0]["impervaCertificate"]["delegation"]["allowedDomainsForCNAMEValidation"]) == 0:
        ...
        for i in range(len(response_dict["data"][0]["impervaCertificate"]["delegation"]["allowedDomainsForCNAMEValidation"])):
            if response_dict["data"][0]["impervaCertificate"]["delegation"]["allowedDomainsForCNAMEValidation"][i]["name"] == domainNameStripped:
...
def getCnameRecord(siteId, accountId, siteName):
    ...
    if len(response_dict["data"][0]["impervaCertificate"]["delegation"]["allowedDomainsForCNAMEValidation"]) == 0:
        ...
        for i in range(
                len(response_dict["data"][0]["impervaCertificate"]["delegation"]["allowedDomainsForCNAMEValidation"])):
            if response_dict["data"][0]["impervaCertificate"]["delegation"]["allowedDomainsForCNAMEValidation"][i][
                "name"] == domainNameStripped:
...
def main():
   ...
   print("### CERTIFICATE STATUS | ACCOUNT ID: " + account_id + " ###")

   #Almacenamos en dataMatrix la matriz con los datos
   dataMatrix = storageMatrix()
